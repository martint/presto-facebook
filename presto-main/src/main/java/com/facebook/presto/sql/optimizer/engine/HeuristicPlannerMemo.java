...
public class HeuristicPlannerMemo
{
    ...
    public boolean transform(Expression from, Expression to, String reason)
    {
        ...
        if (insertion.getGroup() != group) {
            mergeInto(group, insertion.getGroup());
        }

        Map<Expression, VersionedItem<String>> targets = transformations.computeIfAbsent(canonicalize(from), e -> new HashMap<>());
        boolean exists = targets.containsKey(insertion.getExpression());
        targets.computeIfAbsent(insertion.getExpression(), e -> new VersionedItem<>(reason, version++));
        expressionVersions.put(insertion.getExpression(), version++);
        ...
        if (expression instanceof Apply) {
            ...
            Function<Expression, Expression> processor = argument -> {
                if (argument instanceof GroupReference) {
                    ...
                    return new GroupReference(merges.find(((GroupReference) argument).getId()));
                }
...
//    }
    ...
    public Expression canonicalize(Expression expression)
    {
        if (expression instanceof Apply) {
            ...
            return new GroupReference(merges.find((((GroupReference) expression).getId())));
        }
    ...
    public void mergeInto(long targetGroup, long group)
    {
        ...
        while (!pending.isEmpty()) {
            ...
            merges.union(from, to);

            // schedule merges for new equivalences
            for (long x : merges.findAll(from)) {
                Map<Long, List<Expression>> referrerGroups = incomingReferences.get(x)
                        .keySet().stream()
                ...
                for (Map.Entry<Long, List<Expression>> entry : referrerGroups.entrySet()) {
                    ...
                    for (Expression referrerExpression : entry.getValue()) {
                        ...
                        if (formerGroup != null && !formerGroup.equals(referrerGroup)) {
