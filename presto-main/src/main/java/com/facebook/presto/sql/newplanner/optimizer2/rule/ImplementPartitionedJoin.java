...
public class ImplementPartitionedJoin
        implements ImplementationRule
{
    ...
    public List<RelExpr> optimize(RelExpr expression, PhysicalConstraints requirements, OptimizerCallback optimizer)
    {
        ...
        PhysicalConstraints constraint = PhysicalConstraints.partitioned((List<Integer>) expression.getPayload());

        RelExpr optimizedLeft = optimizer.optimize(left, constraint);
        RelExpr optimizedRight = optimizer.optimize(right, constraint);
